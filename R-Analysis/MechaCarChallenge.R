...
MechaCar_mpg <-read.csv(file.choose("MechaCar_mpg.csv"), check.names = F, stringsAsFactors = F)
...
#7. Save your MechaCarChallenge.RScript file to your GitHub repository.
  ...
  Suspension_Coil <-read.csv(file.choose('Suspension_Coil.csv'), check.names = F, stringsAsFactors = F)


#3. Write an RScript that creates a total_summary dataframe using the summarize() function to get the mean, median, variance, and standard deviation of the suspension coil's PSI column.

total_summary <- Suspension_Coil %>% group_by(Mean=mean(PSI),Median=median(PSI),Variance=var(PSI),SD=sd(PSI))
